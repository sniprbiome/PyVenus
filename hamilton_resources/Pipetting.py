...
class Pipetting:
    def __init__(self, con: Connection):
        self.__con = con
        self.__con.execute(definitions=r'#include "C:\Program Files (x86)\HAMILTON\pyVenus_test\smt\Pipetting.hs_"')

    def TransferBuffer(self, star_device: Device, source: Sequence, target: Sequence, tips: Sequence, volume, number_of_transfers: Variable):
        """
        Python function generated from submethod 'TransferBuffer' in 'Pipetting.smt'
...
Tips are reused for all transfers and then discarded
        ...
        self.__con.execute("PIPETTING::TransferBuffer(" + parameter_string + ");")
