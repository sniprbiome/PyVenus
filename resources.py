...
def read_layout(layout):
    ...
    # # generate python definition of deck layout
    # code = "# Generated code to initialize sequences\n"
    # for sequence in sequences:
    #     code += "        self.seq" + sequence + " = self._Sequence('ML_STAR." + sequence + "')\n"
    #
    # # read template for generating deck layout python definition
    # with open(layout_template, "r") as f:
    #     template = f.read()
    #
    # # add code to template
    # template = template.replace("#((*SEQUENCE_PLACEHOLDER*))#", code)
    #
    # # add path to original layout file to template
    # template = template.replace("#((*FILE_PLACEHOLDER*))#", "self.layout_file = r\"" + layout + "\"\n")
    #
    # # write python deck layout file
    # with open(layout_py, 'w') as f:
    #     f.write(template)


def read_liquid_classes(include_1ml_channels=True, include_5ml_channels=False, include_96head=True,
    ...
    for row in cursor.fetchall():
        ...
        # process this row?
        if select:
            code += "        self.lc" + row[1] + " = self._LiquidClass('" + row[1] + "')\n"
...
def read_submethods(directory=""):
    ...
    for path in pathlist:
        ...
        smt_definition = _parse_hxcfg(os.path.join(path.parent, path.stem + ".smt"))
        ...
        for f in match_functions:
            ...
            submethod_comment, parameter_comments = _get_comments(smt_definition, f.group(1))
            ...
            for p in match_parameters:
                ...
                # name: the parameter is not shown in python. An HSL object with the same name as the input parameter is expected to already exist (DEPRECATED)
...
def _get_comments(smt_definition, submethod):
